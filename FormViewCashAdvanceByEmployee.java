 ...
 * @author IT-Earl
 */
public class FormViewCashAdvanceByEmployee extends JInternalFrame{
    private final JTextField txtId, txtName;
    private final JButton btnSearch, btnAdd, btnEdit, btnDelete, btnCancel;
    JTable tableCashAdvance;
    DefaultTableModel tableCashAdvanceModel;
    
    public FormViewCashAdvanceByEmployee(){
        ...
        String[] tableHeader = {
            "ID", "Cash Advance: Amount", "Date"
        };
        tableCashAdvanceModel = new DefaultTableModel();
        tableCashAdvanceModel.setColumnIdentifiers(tableHeader);
        tableCashAdvance = new JTable(){
            @Override
            public boolean isCellEditable(int row, int column) {
                return false;
            }
        };
        tableCashAdvance.setModel(tableCashAdvanceModel);
        tableCashAdvance.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        tableCashAdvance.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
        tableCashAdvance.getColumnModel().getColumn(0).setPreferredWidth(90);
        tableCashAdvance.getColumnModel().getColumn(1).setPreferredWidth(200);
        tableCashAdvance.getColumnModel().getColumn(2).setPreferredWidth(150);
        
        JScrollPane scrollPane = new JScrollPane(tableCashAdvance);
        ...
        setTitle("Cash Advance: Breakdown");
        setBounds(500, 100, 580, 500);
    ...
    private class ButtonFunction implements ActionListener{
        ...
        private void _generate() {
            ...
            tableCashAdvanceModel.setRowCount(0);
            
            try {
                Statement st = conn.createStatement();
                ResultSet rs = st.executeQuery("SELECT CashAdvance_Id, ROUND(CashAdvance_Amount, 2), CashAdvance_Date"
                        + " FROM tbl_cashadvance WHERE Employee_Id='"+txtId.getText()+"'");
                NumberFunction nFunction = new NumberFunction();
                while(rs.next()){
                    String id = rs.getString("CashAdvance_Id");
                    String amount = "Php " + nFunction._getFormattedNumber(rs.getString("ROUND(CashAdvance_Amount, 2)"));
                    String date = rs.getString("CashAdvance_Date");
                    tableCashAdvanceModel.addRow(new Object[]{id, amount, date});
                }
        ...
        private void _add() {
            SubFormAddCashAdvance subFormAddCashAdvance = new SubFormAddCashAdvance(txtId.getText(), txtName.getText());
            if(JOptionPane.showConfirmDialog(null, subFormAddCashAdvance, "Add Cash Advance", 
                    JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.OK_OPTION){
                if(subFormAddCashAdvance._add()){
                    _generate();
                }
            }
        }
        ...
        private void _edit() {
            try {
                NumberFunction nFunction = new NumberFunction();
                SubFormEditCashAdvance subFormEditCashAdvance = new SubFormEditCashAdvance(
                        txtId.getText(),
                        txtName.getText(),
                        tableCashAdvance.getValueAt(tableCashAdvance.getSelectedRow(), 0).toString(),
                        tableCashAdvance.getValueAt(tableCashAdvance.getSelectedRow(), 2).toString(),
                        nFunction._stripValue(tableCashAdvance.getValueAt(tableCashAdvance.getSelectedRow(), 1).toString())
                );
                if(JOptionPane.showConfirmDialog(null, subFormEditCashAdvance, "Edit Cash Advance",
                        JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.OK_OPTION){
                    if(subFormEditCashAdvance._edit()){
                        _generate();
                    }
                }
            } catch(Exception ex){
        ...
        private boolean _goDelete() {
            ...
            try {
                Statement st = conn.createStatement();
                st.executeUpdate("DELETE FROM tbl_cashadvance"
                        + " WHERE Employee_Id='"+txtId.getText()+"'"
                        + " AND CashAdvance_Id='"+tableCashAdvance.getValueAt(tableCashAdvance.getSelectedRow(), 0).toString()+"'");
            } catch (SQLException ex) {
                JOptionPane.showMessageDialog(null, ex.getMessage(), "ERROR", JOptionPane.ERROR_MESSAGE);
                return false;
            } finally {
